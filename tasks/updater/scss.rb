class Updater
  module Scss
    def update_scss_assets
      ...
      contents = {}
      melodic_scss_files = get_paths_by_type('scss', /\.scss$/)
      read_files('scss', melodic_scss_files).each do |name, file|
        contents[name] = file
        save_file("#{save_to}/#{name}", file)
      end
      log_processed((melodic_scss_files * ' ').to_s)

      log_status 'Updating scss main files'
      %w[melodic melodic-grid melodic-reboot].each do |name|
        ...
        File.write to, File.read(to).gsub(/ "/, ' "melodic/')
      end
    end
  end
end
